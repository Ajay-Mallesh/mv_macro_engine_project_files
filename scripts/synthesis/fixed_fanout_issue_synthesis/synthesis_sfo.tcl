...
# AUTHOR      : AJAYMALLESH
# VERSION     : 5.0 (WLM Compatible, Load Isolation Constraints Added)
# DESCRIPTION : Script for SYNTHESIS (dc_shell)
# sfo 		  : splitted fan outs 
# ==========================================================================

suppress_message {OPT-1209 TIM-134}
suppress_message UCN-4

# Create Directory 
set dir_path "./outputs/fixed_high_fanouts"

if {![file exists $dir_path]} {
    file mkdir $dir_path
    puts "Directory created: $dir_path"
} else {
    puts "Directory already exists: $dir_path"
}

# sh mkdir ./outputs/fixed_high_fanouts
...
analyze -format sverilog { \
    ...
    ./inputs/mv_macro_engine_top_sfo.v \
}
...
# ADD THIS EXACT LINE:
# This tells DC: "Map everything to real silicon, but do NOT merge equivalent registers!"
...
# THE FIX: Force a buffer tree based on raw pin count, bypassing the 0-cap blindness.
# No single wire will be allowed to touch more than 16 pins.
set_max_fanout 16 [get_designs mv_macro_engine_top]

compile_ultra -scan -no_autoungroup -gate_clock -retime

# Insert level shifters and isolation cells for your 0.95V/0.75V domains
insert_mv_cells
...
# --- A. SCANDEF ---
write_scan_def -output ./outputs/fixed_high_fanouts/mv_macro_engine_top.scandef

# --- B. UPF ---
save_upf ./outputs/fixed_high_fanouts/mv_macro_engine_top.upf

set file_name "./outputs/fixed_high_fanouts/mv_macro_engine_top.upf"
...
write_sdc ./outputs/fixed_high_fanouts/mv_macro_engine_top_synth.sdc

set file_name "./outputs/fixed_high_fanouts/mv_macro_engine_top_synth.sdc"
...
# --- D. NETLIST ---
write -format verilog -hierarchy -output ./outputs/fixed_high_fanouts/mv_macro_engine_top_netlist_sfo.v
set file_name "./outputs/fixed_high_fanouts/mv_macro_engine_top_netlist_sfo.v"
set in_file [open $file_name r]; set file_data [read $in_file]; close $in_file
set out_file [open $file_name w]
puts $out_file "// =========================================================================="
puts $out_file "// PROJECT     : Multi-Voltage Macro Engine"
